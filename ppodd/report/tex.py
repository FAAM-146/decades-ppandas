...
from pylatex.utils import escape_latex as _el  # type: ignore
    ...
    def _get_meta(self):
        ...
        try:
            self.takeoff_time = datetime.datetime.strptime(
                meta["takeoff"]["time_utc"], "%Y-%m-%dT%H:%M:%S"
            )
        ...
        try:
            self.landing_time = datetime.datetime.strptime(
                meta["landing"]["time_utc"], "%Y-%m-%dT%H:%M:%S"
            )
        except Exception:
            pass
...
""".replace(
            ...
            .replace("TAG_PROJECT", self.project)
            .replace("TAG_DATE", self.date.strftime("%A, %B %-d, %Y"))
        )
    ...
    def _flight_summary(self):
        ...
        _retstr += r"\hline" + "\n"
        _retstr += (
            r"Event & Start & Start lat & Start lon & Start alt & "
    ...
    def _qaqc(self):
        ...
        for _file in glob.glob(os.path.join(self.qa_dir, "*.pdf")):
            _retstr += (
                r"\includepdf[pages=-]{" + _file.replace(".pdf", "") + r"}" + "\n"
            )
    ...
    def _overview(self):
        ...
        flight_data = pd.DataFrame(
            {"alt": alt, "lat": lat, "lon": lon}, index=alt.index
        )
        try:
        ...
        shutil.move(
            ...
            os.path.join(
                ...
                "flight-report_faam_{date}_r0_{fltnum}.pdf".format(
                    date=self.date.strftime("%Y%m%d"), fltnum=self.flight_number.lower()
                ),
            ),
        )
