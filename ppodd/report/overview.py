...
import cartopy.crs as ccrs  # type: ignore  # no type hints
...
from gribgrab import GFSDownloader  # type: ignore


def get_data(cycle: int, horizon: int, wgrib2: str = "wgrib2") -> None:
...
def nearest(array: Iterable, value: int) -> int:
    array = np.asarray(array)
    idx = (np.abs(array - value)).argmin()
    return idx


def add_text(ax: Axes, meta: dict) -> None:

    ax.text(0, 0.6, "Flight: {}".format(meta["flight_number"]))
    ax.text(0, 0.4, "Project: {}".format(meta["project"]))

    ax.text(0, 0, "Depart: {}".format(meta["takeoff"]["location"]))
    ax.text(0, -0.2, "Arrive: {}".format(meta["landing"]["location"]))
    ax.text(0, -0.4, "Takeoff: {}".format(meta["takeoff"]["time_utc"]))
    ax.text(0, -0.6, "Land: {}".format(meta["landing"]["time_utc"]))

    ax.text(0.5, 0.6, "Lat max: {0:0.2f} °N".format(meta["domain"]["lats"][1]))
    ax.text(0.5, 0.4, "Lat min: {0:0.2f} °N".format(meta["domain"]["lats"][0]))
    ax.text(0.5, 0.2, "Lon max: {0:0.2f} °E".format(meta["domain"]["lons"][1]))
    ax.text(0.5, 0, "Lon max: {0:0.2f} °E".format(meta["domain"]["lons"][0]))
    ax.text(0.5, -0.4, "Ceiling: {0:d} m".format(meta["domain"]["max_alt"]))


def make_plot(
    flight_data: pd.DataFrame, outfile: str = "overview.pdf", meta: dict | None = None
) -> None:
    ...
    if lon_range < 0.75 * lat_range:
        delta = (lat_range - lon_range) / 2
        lon_min -= delta
        lon_max += delta

    if lat_range < 0.75 * lon_range:
    ...
    OCEAN = cfeature.NaturalEarthFeature(
        "physical", "ocean", "50m", edgecolor="face", facecolor=cfeature.COLORS["water"]
    )

    fig = plt.figure(figsize=(8, 8 * 1.414))

    ax = fig.add_axes((0.1, 0.5, 0.8, 0.4), projection=ccrs.PlateCarree())
    ax2 = fig.add_axes((0.1, 0.25, 0.8, 0.2))
    ...
    ax.add_feature(OCEAN) # type: ignore
    ax.add_feature(cfeature.LAND) # type: ignore
    ...
    ax.set_title(
        "Synoptic Overview (GFS {cycle} T+{tplus})".format(
    ...
    ax2.plot(flight_data.alt, linewidth=3, color=[0.7, 0, 0])
