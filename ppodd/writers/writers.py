...
from ppodd.utils import pd_freq, unwrap_array, mogrify_attribute
...
class NetCDFWriter(DecadesWriter):
    ...
    def _write_var(self, nc, var):
        ...
        if _freq == 1:
            ncvar = nc.createVariable(var.name, "f4", ("Time",), fill_value=fill_value)
        ...
        for attr_key, attr_val in var.attrs().items():
            ...
            if attr_val == ATTRIBUTE_NOT_SET:
                logger.warning(
                    "Unset variable attribute: {}.{}".format(var.name, attr_key)
                )
                continue

            try:
                setattr(ncvar, attr_key, mogrify_attribute(attr_val))
            except TypeError:
                logger.error(
                    f"Error setting variable attr {attr_key} = {attr_val} on {var.name}"
                )
        ...
        _index = pd.date_range(self.start_time, _end, freq=pd_freq[_freq])
        ...
        if _freq == var.frequency:
            ...
            if var.flag is not None:
                _flag = var.flag().reindex(_index).fillna(flag_fill_value)
        else:
            ...
            if var.flag is not None:
                _flag = (
                    var.flag()
                    .resample(pd_freq[_freq])
                    .mean()
                    .pad()
                    .reindex(_index)
                    .fillna(flag_fill_value)
                )
    ...
    def _write_global(self, nc, key, value):
        ...
        # Coerce attribute value to something netCDF can handle
        value = mogrify_attribute(value)
    ...
    def write(self, filename=None, freq=None):
        ...
        if filename is None:
            ...
            if freq is None:
                _freq = ""
            filename = "core_faam_{date}_v005_r{revision}_{flight}{freq}.nc".format(
        ...
        with Dataset(filename, "w", format=self._format) as nc:
            ...
            dates = pd.date_range(self.start_time, self.end_time, freq="s")
            ...
            nc["Time"][:] = [
                i / 1e9 - _delta_secs for i in dates.values.astype(np.int64)
            ]

            # Set the ID of the dataset before writing to file
            self.dataset.data_id = os.path.basename(filename.replace(".nc", ""))
