...
class GINQA(QAMod):
    ...
    def make_map(self, fig):
        """
        Make a map showing the flight track.
        """
        _ax = fig.axes([.1, .6, .8, .31])

        m = Basemap(projection='tmerc', urcrnrlat=self.lat.max()+1.5,
                    urcrnrlon=self.lon.max()+3, llcrnrlat=self.lat.min()-1.5,
                    llcrnrlon=self.lon.min()-3, resolution='i',
                    lat_0=self.lat.mean(), lon_0=self.lon.mean())

        m.plot(self.lon.values, self.lat.values, latlon=True, linewidth=3,
               color='#cc0000')
        m.drawcoastlines()
        m.fillcontinents()
        m.drawcountries()
    ...
    def make_gin_wind_plot(self, fig):
        """
        Create a timeseries plot showing the noturb wind speed and direction.
        """
        _ax, _ax2 = fig.timeseries_axes([.1, .12, .8, .15], twinx=True)
    ...
    def run(self):
        ...
        with QAFigure(self.dataset, 'GIN') as fig:
            ...
            # Make a common index
            _index = (_wow.index.intersection(_lat.index)
                                .intersection(_alt_rvs.index))
