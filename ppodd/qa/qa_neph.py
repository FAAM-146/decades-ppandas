...
class NephQA(QAMod):
    ...
    inputs = [
        'TSC_BLUU', 'TSC_GRNU', 'TSC_REDU',
        'BSC_BLUU', 'BSC_GRNU', 'BSC_REDU',
        'NEPH_PR', 'NEPH_T', 'NEPH_RH',
        'PS_RVSM'
    ]

    def make_backscatter_timeseries(self, fig):
        _ax = fig.timeseries_axes([.1, .7, .8, .2], labelx=False)

        _ax.plot(self.dataset['BSC_REDU'](), color='tab:red', alpha=.5)
        _ax.plot(self.dataset['BSC_GRNU'](), color='tab:green', alpha=.5)
        _ax.plot(self.dataset['BSC_BLUU'](), color='tab:blue', alpha=.5)
    ...
    def make_total_timeseries(self, fig):
        _ax = fig.timeseries_axes([.1, .5, .8, .2], labelx=False)

        _ax.plot(self.dataset['TSC_REDU'](), color='tab:red', alpha=.5)
        _ax.plot(self.dataset['TSC_GRNU'](), color='tab:green', alpha=.5)
        _ax.plot(self.dataset['TSC_BLUU'](), color='tab:blue', alpha=.5)
    ...
    def make_temp_timeseries(self, fig):
        _ax, _ax2 = fig.timeseries_axes([.1, .3, .8, .2], labelx=False,
                                        twinx=True)
    ...
    def make_press_timeseries(self, fig):
        _ax = fig.timeseries_axes([.1, .1, .8, .2])
