...
class CPCQA(QAMod):
    ...
    def make_flow_timeseries(self, fig: QAFigure) -> None:
    ...
    def make_temp_timeseries(self, fig: QAFigure) -> None:
        ax = fig.timeseries_axes([0.1, 0.5, 0.8, 0.2])
        ax.plot(self.dataset["CPC378_saturator_temp"], "r", label="Saturator temp")
        ax.legend(loc="lower left", fontsize=8)
    ...
    def make_counts_timeseries(self, fig: QAFigure) -> None:
        ax = fig.timeseries_axes([0.1, 0.15, 0.8, 0.3])
        ax2 = ax.twinx()
        ax2.yaxis.label.set_fontsize(6)
        for label in ax2.get_yticklabels():
            label.set_fontsize(6)
