...
class TwoBOzone(QAMod):
    ...
    def make_conc_plots(self, fig: QAFigure) -> None:
        have_teco = True

        ax1 = fig.timeseries_axes([0.1, 0.73, 0.5, 0.2])
        ax2 = fig.axes([0.65, 0.73, 0.25, 0.2])

        ax1.plot(fig.filter_in_flight(self.dataset["O3_2BTECH"]()), label="2BTech")
        ...
        try:
            ...
            ax1.plot(fig.filter_in_flight(teco), label="TECO")
        ...
        if not have_teco:
            ax2.set_xticks([])
            ax2.set_yticks([])
            ax2.text(0.1, 0.5, "O3_TECO not available")
            return
    ...
    def make_flow(self, fig: QAFigure) -> None:
        ax1, ax2 = fig.timeseries_axes([0.1, 0.55, 0.8, 0.15], twinx=True, labelx=False)
    ...
    def make_press(self, fig: QAFigure) -> None:
        ax1, ax2 = fig.timeseries_axes([0.1, 0.4, 0.8, 0.15], twinx=True, labelx=False)
        ax1.plot(
            fig.filter_in_flight(self.dataset["TWBOZO_press"]),
            color="tab:green",
            label="2B Pressure",
        )
        ax1.legend(loc="lower left", fontsize=6)
        ...
        try:
            ax2.plot(
                fig.filter_in_flight(self.dataset["CAB_PRES"]) * 0.750062,
                color="tab:purple",
    ...
    def make_temp(self, fig: QAFigure) -> None:
        ax1 = fig.timeseries_axes([0.1, 0.25, 0.8, 0.15])
        ax1.plot(fig.filter_in_flight(self.dataset["TWBOZO_temp"]), label="2B temp.")
        ax1.plot(
            fig.filter_in_flight(self.dataset["TWBOZO_cRIO_temp_c"]),
            label="2B crio temp.",
        )
        ax1.set_ylabel("Temp (C)")
        ax1.legend(loc="lower left", fontsize=6)

    def make_zero(self, fig: QAFigure) -> None:
        ax = fig.axes([0.1, 0.07, 0.8, 0.15])

        try:
            v6 = self.dataset["TWBOZO_V6"]()
        except KeyError:
            ax.text(0.35, 0.5, "2B Valve state not available")
            ax.set_xticks([])
            ax.set_yticks([])
            return
        ...
        if in_zero.max() == 0:
            ax.text(0.37, 0.5, "No zeroes were performed")
            ax.set_xticks([])
            ax.set_yticks([])
            return
        ...
        ax.fill_between([0, m], -3, 3, color="tab:green", alpha=0.3)
        ax.set_xlim([0, m])
        ax.set_ylim([-5, 5])
