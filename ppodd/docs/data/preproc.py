...
from ppodd.pod.base import PPBase, pp_register
...
def trim_docstr(docstring: str) -> str:
    """
...
def get_global_attrs_string(required: bool = True, standard=None):
    """
    ...
    if required:
        attrs = {
            k: v for k, v in schema["properties"].items() if k in schema["required"]
        }
    else:
        attrs = {
            k: v for k, v in schema["properties"].items() if k not in schema["required"]
        }
...
def get_variable_attrs_string(required=True, standard=None):
    """
...
def get_module_doc(module: type[PPBase]) -> str:
    """
...
def get_flagmod_doc(module: type[FlaggingBase]) -> str:
    """
    ...
    for var in module.flagged:
        v = DecadesVariable(
            pd.Series(flag, index=index, name=var), flag=DecadesBitmaskFlag
        )
        d.add_output(v)
...
def get_module_vardoc(module: PPBase) -> str:
    """
...
def get_module_flagdoc(module: PPBase) -> str:
    """
    ...
    if type(module.dataset[var].flag) is DecadesBitmaskFlag:
        output += (
            "Variables in this module use bitmask flags. Some, all or "
        ...
        output += "Variables in this module use classic, value based " "flagging.\n"
    ...
    if type(module.dataset[var].flag) is DecadesBitmaskFlag:
        ...
        output += "\n"
        for dvar in module.dataset.outputs:
            output += f"* {dvar.name}_FLAG\n"
            meanings = module.dataset[dvar.name].flag.cfattrs["flag_meanings"]
            meanings = meanings.split()

            values = module.dataset[dvar.name].flag.cfattrs["flag_values"]
            for value, meaning in zip(values, meanings):
                output += "    * ``{}``: ``{}`` - {}\n".format(
                    value, meaning, module.dataset[dvar.name].flag.descriptions[value]
                )
            output += "\n"
...
def get_variables_tex(default: bool = True) -> str:
    """
...
def get_dimensions() -> str:
    ...
    _ldims = sorted(list(_dims))

    for dim in _ldims:
        dimstr += (
            f"* ``{dim[0]}`` - {dim[1]} samples per second. "
...
def get_variables_web(default=True) -> str:
    """
...
def replace_tag(path: str, tag: str, content: str) -> None:
    """
...
if __name__ == "__main__":
    ...
    replace_tag(
        core, "TAG_REQUIRED_GLOBAL_ATTRIBUTES", get_global_attrs_string(True, standard)
    )

    replace_tag(
        core, "TAG_OPTIONAL_GLOBAL_ATTRIBUTES", get_global_attrs_string(False, standard)
    )
    ...
    for fmod in flag_modules:
        append(flagmods, get_flagmod_doc(fmod))
