...
class NasaAmes1001Reader(object):
    def __init__(self, ames_file):
        ...
        if self.ffi != FFI_1001:
            raise ValueError(
                f'Can only read FFA={FFI_1001} files. This file is {ffa}.'
            )
    ...
    def read(self):
        ...
        self.meta['ivol'], self.meta['nvol'] = [
            int(i) for i in header.pop(0).split()
        ]

        _dates = header.pop(0).split()
        self.meta['date'] =  datetime.datetime.strptime(
            ''.join(_dates[:3]), DFMT
        )
        self.meta['rdate'] = datetime.datetime.strptime(
            ''.join(_dates[3:]), DFMT
        )
        ...
        data = pd.read_csv(
            self.ames_file, skiprows=self.nlhead, header=None,
            delimiter=' ', index_col=0, names=self.meta['vname']
        )
        ...
        data.index = [self.meta['date'] + datetime.timedelta(seconds=i)
                      for i in data.index]
