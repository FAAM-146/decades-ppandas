...
from ..utils.calcs import true_air_temp_variable
...
class PRTTemperatures(PPBase):
    ...
    inputs = [
        ...
        "SH_GAMMA",  #  Ratio of specific heats (Derived)
        "MACH",  #  Mach number (Derived)
        "ETA_ND",  # Recovery factor (Derived)
        "ETA_DI",  # Recovery factor (Derived)
    ...
    def test():
        ...
        return {
            ...
            "MACH": ("data", 0.5 * _o(700), 32),
            "ETA_ND": ("data", _z(700), 32),
            "ETA_DI": ("data", _z(700), 32),
            "CORCON_di_temp": ("data", _a(225, 245, 0.0286) * 1000, 32),
            "CORCON_ndi_temp": ("data", _a(225, 245, 0.0286) * 1000, 32),
            "PRTAFT_deiced_temp_flag": ("data", _c([_z(200), _o(300), _z(200)]), 32),
    ...
    def declare_outputs(self):
        ...
        sampling = (
            "Sensor housed in Rosemount Aerospace Inc. Type 102 "
        ...
        try:
            ...
            message = EM_CANNOT_INIT_MODULE.format(
                module_name=self.__class__.__name__, constants="DITSENS, NDTSENS"
            )
        ...
        if self.test_mode or self.dataset["DITSENS"][1].lower() != "thermistor":
            self.declare(
                ...
                frequency=32,
                long_name=(
                    "True air temperature from the Rosemount deiced "
                    "temperature sensor"
                ),
            ...
            self.declare(
                ...
                frequency=32,
                long_name=(
                    "Indicated air temperature from the Rosemount deiced "
                    "temperature sensor"
                ),
                sensor_type=self.dataset.lazy["DITSENS"][1],
                sensor_serial_number=self.dataset.lazy["DITSENS"][0],
        ...
        if self.test_mode or self.dataset["NDTSENS"][1].lower() != "thermistor":
            self.declare(
                ...
                frequency=32,
                long_name=(
                    "True air temperature from the Rosemount non-deiced "
                    "temperature sensor"
                ),
                standard_name="air_temperature",
            ...
            self.declare(
                ...
                frequency=32,
                long_name=(
                    "Indicated air temperature from the Rosemount "
    ...
    def calc_heating_correction(self):
        ...
        heating_flag = d["PRTAFT_deiced_temp_flag"].ffill().fillna(0)
    ...
    def calc_ndi_tat(self):
        ...
        d["TAT_ND_R"] = true_air_temp_variable(
            d.IAT_ND_R, d.MACH, d.ETA_ND, d.SH_GAMMA.interpolate(limit=32)
        )
    ...
    def calc_di_tat(self):
        ...
        d["TAT_DI_R"] = true_air_temp_variable(
            d.IAT_DI_R, d.MACH, d.ETA_DI, d.SH_GAMMA.interpolate(limit=32)
        )
