...
class BBRFlux(PPBase):
    ...
    def test() -> TestData:
        ...
        return {
            ...
            "UP2Z": ("data", _z(100), 1),
            "UIRZ": ("data", _z(100), 1),
            "UP1T": ("data", 250 * _o(100), 1),
            "UP2T": ("data", 250 * _o(100), 1),
            "UIRT": ("data", 400 * _o(100), 1),
            "LP1S": ("data", 300 * _o(100), 1),
            "LP2S": ("data", 150 * _o(100), 1),
            "LIRS": ("data", _z(100), 1),
            "LP1Z": ("data", _z(100), 1),
            "LP2Z": ("data", _z(100), 1),
            "LIRZ": ("data", _z(100), 1),
            "LP1T": ("data", 250 * _o(100), 1),
            "LP2T": ("data", 250 * _o(100), 1),
            "LIRT": ("data", 400 * _o(100), 1),
            "SOL_AZIM": ("data", _l(200, 250, 100), 1),
            "SOL_ZEN": ("data", _l(25, 50, 100), 1),
            "ROLL_GIN": ("data", _c([_l(0, 5, 50), _l(5, 0, 50)]), 32),
            "PTCH_GIN": ("data", 6 * _o(100), 32),
    ...
    def process(self) -> None:
        ...
        self.get_dataframe(
            method="onto", index=self.dataset["UP1S"].index, circular=["HDG_GIN"]
        )
        d = self.d
        ...
        # radiation
        d["FCRIT"] = 920.0 * (np.cos(d.ZENRAD)) ** 1.28

        ceff = np.array(
            [1.010, 1.005, 1.005, 1.005, 1.000, 0.995, 0.985, 0.970, 0.930, 0.930]
        )

        fdir = np.array([0.95] * 10)
        ...
        for dome in ("P1", "P2"):
            for pos in ("U", "L"):
                ...
                _calname = "CALC{pos}{ins}F".format(pos=pos, ins=_caldict[dome])
                ...
                cos_beta = (
                    ...
                    - np.cos(roll) * np.sin(pitch) * np.sin(d.ZENRAD) * np.cos(d.SUNHDG)
                )
                ...
                _temp = d[_flux] / (1.0 + th * (tsa + th * (tsb + th * tsg)))
                d[_flux] = _temp
                ...
                d["_ceff"] = np.array(
                    [ceff[min(int(i), 9)] if np.isfinite(i) else np.nan for i in index]
                )

                d["_fdir"] = np.array(
                    [fdir[min(int(i), 9)] if np.isfinite(i) else np.nan for i in index]
                )
                ...
                if pos == "U":
                    _above_crit = d[_flux] / (
                        1.0
                        - (d._fdir * (1.0 - d._ceff * (cos_beta / np.cos(d.ZENRAD))))
                    )
                    _direct = d[_flux] > fcritval
                    d.loc[_direct, _flux] = _above_crit[_direct]

                output_dome_dict = {"P1": "SW", "P2": "RED"}

                output_pos_dict = {"U": "DN", "L": "UP"}

                output_name = "{wavelength}_{pos}_C".format(
                    wavelength=output_dome_dict[dome], pos=output_pos_dict[pos]
                )
                ...
                flux_limit_lo = FLUX_LIMITS[
                    "{}_{}_MIN".format(output_pos_dict[pos], output_dome_dict[dome])
                ]

                flux_limit_hi = FLUX_LIMITS[
                    "{}_{}_MAX".format(output_pos_dict[pos], output_dome_dict[dome])
                ]
