"""
Provides a postprocessing module which provides geographical info for the CCN
...
class CCNGeo(PPBase):
    ...
    resolution of data from the CCN.
    """
    ...
    def test():
        ...
        """
        return {
            'ccn_column_a_conc': ('data', 400 * _o(100), 1),
    ...
    def process(self):
        ...
        """
        index = self.dataset['ccn_column_a_conc'].index
        ...
        for meaning in lat_flag.meanings:
            try:
                ...
                lat.flag.add_mask((lat.data == 0) | (lon.data == 0),
                                   'latlon identically zero')

        for meaning in lon_flag.meanings:
            try:
                lon.flag.add_mask(lon_flag_df[meaning], meaning)
            except (KeyError, AttributeError):
                lon.flag.add_mask((lat.data == 0) | (lon.data == 0),
                                   'latlon identically zero')
        ...
        for meaning in alt_flag.meanings:
            try:
                ...
                alt.flag.add_mask((lat.data == 0) | (lon.data == 0),
                                   'latlon identically zero')

        self.add_output(lat)
        self.add_output(lon)
        self.add_output(alt)
        self.dataset.lat = 'latitude'
        self.dataset.lon = 'longitude'
